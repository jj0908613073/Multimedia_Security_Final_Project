...
def attack_and_evaluate(attack_type, original_image):
    ...
    attacked_image_path = os.path.join('LSB-Steganography/result', attacked_image_filename)
...
def main():
    # 載入原始圖像(修改過後)
    original_image = cv2.imread("LSB-Steganography/image/res_encode.png", 0)
    ...
    plt.savefig(os.path.join('LSB-Steganography/result', 'all_results.png'))
    plt.show()

    # Write results to a text file
    with open('LSB-Steganography/result/attack_results.txt', 'w') as f:
        f.write('Attack Results for LSB-Steganography\n\n')
